...
// ✅ Logo koji želiš koristiti (RAW link s GitHuba)
const logoUrl =
  'https://raw.githubusercontent.com/malimujo/HRT-Poslovni-tjedan/main/poslovnitjedan.png';
...
async function updatePovijestM3U() {
  ...
  try {
    ...
    console.log('📄 Učitavam https://radio.hrt.hr/slusaonica/poslovni-tjedan');
    await page.goto('https://radio.hrt.hr/slusaonica/poslovni-tjedan', {
      waitUntil: 'networkidle2',
    });

    await new Promise((r) => setTimeout(r, 4000));

    // 🎯 JSON izvlačenje za NAZIV (caption)
    const result = await page.evaluate(() => {
      ...
      for (const script of scripts) {
        ...
        const jsonMatch = content.match(
          /lastAvailableEpisode[^}]*"caption"\\s*:\\s*"([^"]+)"/
        );
        if (jsonMatch) {
      ...
      // 3. REGEX‑om u scriptovima tražiš MP3
      for (const script of scripts) {
        const content = script.textContent || script.innerHTML;
...
#EXTINF:-1 tvg-logo="${logoUrl}" group-title="Slušaonica",${emisijaInfo}
${result.mp3}`;

      fs.writeFileSync('Poslovni_tjedan.m3u', m3uContent);
      console.log('✅ M3U spreman s fiksanim logom!');
    } else {
...
#EXTINF:-1 tvg-logo="${logoUrl}",HRT Povijest četvrtkom - Projekt Manhattan II. dio 11.03. 20:00
https://api.hrt.hr/media/28/da/20260311-povijest-cetvrtkom-37328740-20260311200000.mp3`;
    fs.writeFileSync('Poslovni_tjedan.m3u', fallbackContent);
